module.exports = function toReadable (num) {
    ...
    const zero = 'zero';
    const units = ['one', 'two', 'tree', 'four', 'five', 'six', 'seven', 'eight', 'nine'];
    const secondTen = ['eleven', 'twelve', 'thirteen', 'fourteen', 'fiveteen', 'sixteen', 'seventeen', 'eighteen', 'nineteen'];
    const tens = ['ten', 'twenty', 'thirty', 'forty', 'fifty', 'sixty', 'seventy', 'eighty', 'ninety'];
    const hundred = ['one hundred', 'two hundred', 'tree hundred', 'four hundred', 'five hundred', 'six hundred', 'seven hundred', 'eight hundred', 'nine hundred'];
          ...
          if (number.charAt(0) == i) {
            for (let j = 0; j <= units.length; j++) {
              if (number.charAt(1) == j) {
                console.log(tens[i - 1], units[j - 1])
              }
            }
          }
        }
        ...
        for (let i = 0; i <= hundred.length; i++) {
          ...
          if (number.charAt(0) == i) {
            ...
            for (let j = 0; j <= tens.length; j++) {
              ...
              if (number.charAt(1) == j && number.charAt(1) != 1) {
